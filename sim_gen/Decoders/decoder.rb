    ...
    module Helper
      ...
      def calc_insn_mask(insn)
        ...
        for field in insn[:fields]
          unless field[:value][:value_num].nil?
      ...
      def get_lead_bits(instructions, separ_mask = 0)
        ...
        for bit in 0...max_len
          ...
          lead_bits[bit] = [count_0, count_1] if count_0 > 0 && count_1 > 0
        end
        lead_bits
      end
      ...
      def make_child_tree(node_value, separ_mask, instructions, subtree)
        ...
        return nil, false if sublits.empty?
        return sublits[0], true if sublits.size == 1
